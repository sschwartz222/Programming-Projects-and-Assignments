...
;; the world consists of the turtle and the drawing surface
(define-struct World
...
(define (turn d world)
  (World
   (turtle-set-dir (World-turtle world) d)
   (World-surf world)))
...
(: polygon-aux : Integer Integer Integer World -> World)
;; counts upwards from i to n-1, accumlating forward and turn operations
;; helper for polygon
(define (polygon-aux n i d world)
  (cond
    [(< i n) (polygon-aux n (+ i 1) d (turn (* (+ i 1) ( / 360 n)) (forward d world)))]
    [else world]))

(: polygon : Integer Integer (U Image-Color Pen) -> Image)
;; draws a polygon of n sides d side length and pen color
(define (polygon n d pen)
  (if (or (< n 3) (<= d 0))
      (error "not a valid polygon")
      (get-image (polygon-aux n 0 d (make-world (make-turtle pen))))))
...
;;
(test)
(get-image (forward 40 (make-world (make-turtle "blue"))))
(get-image (forward 40
                    (move-by (Point -20 100)
                             (forward 20
                                   (turn 45
                                         (move-to (Point -10 30)
                                                  (make-world
