...
(define (loc->coord lay x y)
  (if (and (<= (Layout-board-left lay) x (Layout-board-right lay))
      ...
      (Some (Coord (truncate (/ (- y (+ (Layout-pad-wid lay) (Layout-cell-wid lay)))
                                (Layout-cell-wid lay)))
                   (truncate (/ (- x (+ (Layout-pad-wid lay) (Layout-cell-wid lay)))
                                (Layout-cell-wid lay)))))
      'None))
...
;;
(define grader-layout-5-6-30
  (Layout 250 315
          5 30 24
          35 215 35 215))
(define grader-layout-5-8-30
  (Layout 310 375
          5 30 24
          35 215 35 215))
(define grader-layout-7-8-40
  (Layout 414 481
          7 40 32
          47 367 47 367))
;; loc->coord
(check-expect (loc->coord grader-layout-5-6-30 0 0) 'None)
(check-expect (loc->coord grader-layout-5-6-30 32 32) 'None)
(check-expect (loc->coord grader-layout-5-6-30 40 40) (Some (Coord 0 0))) 
(check-expect (loc->coord grader-layout-5-6-30 0 200) 'None)
(check-expect (loc->coord grader-layout-5-6-30 160 186) (Some (Coord 5 4))) 
(check-expect (loc->coord grader-layout-5-6-30 216 200) 'None)
(check-expect (loc->coord grader-layout-5-6-30 200 216) 'None)
(check-expect (loc->coord grader-layout-5-6-30 216 216) 'None)
...
(: game-image : Game Layout String -> Image)
...
(define (game-image g lay str)
  (place-image
   ...
   (place-image
    (text str (exact-round (* 0.6 (Layout-text-sz lay))) 'black)
    (/ (Layout-width lay) 2)
    (- (+ (Layout-height lay) (Layout-cell-wid lay))
       (+ (Layout-pad-wid lay) (* 3 (/ (Layout-cell-wid lay) 2))))
    (place-image
     (text
      (string-append
       "Score: White-"
     ...
     (/ (Layout-width lay) 2)
     (- (+ (Layout-height lay) (Layout-cell-wid lay))
        (+ (Layout-pad-wid lay) (/ (Layout-cell-wid lay) 2)))
     (empty-scene (Layout-width lay) (+ (Layout-cell-wid lay) (Layout-height lay))
                  'white)))))
;; eyeball tests below
...
"bigger 10x10 board"
;; game image
(game-image (make-game 6) (make-layout 10 6 40) "testing")
"same 6x6 board as above but with the other stuff in the window"
(game-image (make-game 10) (make-layout 20 10 50) "yippee")
